...
-- 게시판
DROP TABLE IF EXISTS pme_board RESTRICT;
...
-- 작업
DROP TABLE IF EXISTS pme_task RESTRICT;
...
-- 게시판
ALTER TABLE pme_board
  ADD CONSTRAINT PK_pme_board -- 게시판 기본키
    PRIMARY KEY (
      board_no -- 게시글번호
    );
...
-- 작업
CREATE TABLE pme_task (
  task_no        INTEGER  NOT NULL COMMENT '작업번호', -- 작업번호
  project_no     INTEGER  NOT NULL COMMENT '프로젝트번호', -- 프로젝트번호
  member_no      INTEGER  NOT NULL COMMENT '회원번호', -- 회원번호
  content        TEXT     NOT NULL COMMENT '내용', -- 내용
  deadline       DATETIME NOT NULL COMMENT '마감일', -- 마감일
  task_status_no INTEGER  NOT NULL COMMENT '작업상태번호' -- 작업상태번호
)
COMMENT '작업';

-- 작업
ALTER TABLE pme_task
  ADD CONSTRAINT PK_pme_task -- 작업 기본키
    PRIMARY KEY (
      task_no -- 작업번호
    );
...
-- 게시판
ALTER TABLE pme_board
  ADD CONSTRAINT FK_pms_member_TO_pme_board -- 회원 -> 게시판
    FOREIGN KEY (
      member_no -- 회원번호
    )
...
-- 작업
ALTER TABLE pme_task
  ADD CONSTRAINT FK_pms_project_member_TO_pme_task -- 프로젝트멤버 -> 작업
    FOREIGN KEY (
      project_no, -- 프로젝트번호
      member_no   -- 회원번호
    )
...
-- 작업
ALTER TABLE pme_task
  ADD CONSTRAINT FK_pms_task_status_TO_pme_task -- 작업상태 -> 작업
    FOREIGN KEY (
      task_status_no -- 작업상태번호
    )
...
-- 좋아요
ALTER TABLE pms_like
  ADD CONSTRAINT FK_pme_board_TO_pms_like -- 게시판 -> 좋아요
    FOREIGN KEY (
      board_no -- 게시글번호
    )
    REFERENCES pme_board ( -- 게시판
      board_no -- 게시글번호
    );

-- 첨부파일
ALTER TABLE pms_board_file
  ADD CONSTRAINT FK_pme_board_TO_pms_board_file -- 게시판 -> 첨부파일
    FOREIGN KEY (
      board_no -- 게시글번호
    )
    REFERENCES pme_board ( -- 게시판
...
-- 내관심게시글
ALTER TABLE pms_mylist
  ADD CONSTRAINT FK_pme_board_TO_pms_mylist -- 게시판 -> 내관심게시글
    FOREIGN KEY (
      board_no -- 게시글번호
    )
    REFERENCES pme_board ( -- 게시판
...
-- 댓글
ALTER TABLE pms_comment
  ADD CONSTRAINT FK_pme_board_TO_pms_comment -- 게시판 -> 댓글
    FOREIGN KEY (
      board_no -- 게시글번호
    )
    REFERENCES pme_board ( -- 게시판
