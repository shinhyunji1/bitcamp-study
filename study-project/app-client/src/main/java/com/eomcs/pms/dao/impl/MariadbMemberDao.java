...
public class MariadbMemberDao implements MemberDao {
  ...
  public void insert(Member member) throws Exception {
    try (PreparedStatement stmt = con.prepareStatement(// 변수 선언만 가능
        "insert into pms_member(name,email,password,photo,tel) values(?,?,password(?),?,?)")) {// 암호는 단방향으로만 가능
  ...
  public Member findByName(String name) throws Exception {
    try (PreparedStatement stmt = con.prepareStatement(
        "select member_no,name,email,photo,tel,created_dt from pms_member"
            + " where name=?")) { // SQL 삽입 공격이 가능하기 때문에 직접 받지 않는다.
  ...
  public Member findByEmailAndPassword(String email, String password) throws Exception {
    try(PreparedStatement stmt = con.prepareStatement(
        "select member_no,name,email,photo,tel,created_dt from pms_member"
            + " where email=? and password=password(?)")) { // SQL 삽입 공격이 가능하기 때문에 직접 받지 않는다.
